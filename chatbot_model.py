...
def trainIntentModel():
   ...
   # Return a dictionary, mapping labels to their integer values
    ...
    pk.dump(intent_label_map, open("saved_state/intent_label_map.pkl", "wb"))
    ...
    classifier.add(Dense(units = 96, kernel_initializer = 'uniform', activation = 'relu', input_dim = 108))

    # Adding the second hidden layer
    classifier.add(Dense(units = 96, kernel_initializer = 'uniform', activation = 'relu'))

    # Adding the output layer
    classifier.add(Dense(units = 22, kernel_initializer = 'uniform', activation = 'softmax'))
...
def trainEntityModel():
    # Importing dataset and splitting into words and labels
    dataset = pd.read_csv('datasets/data_tags.csv', names=["word", "label"])
...
#     X = cv.fit_transform(X.astype('U')).toarray()
    ...
    pk.dump(entity_label_map, open("saved_state/entity_label_map.pkl", "wb"))
